...
System.out.println(imageUp.getParameter("book_id"));
...
// 책의 정보를 수정하는 프로그램에서는 입력하는 프로그램과 다르게 어떤 책인 지를 가리키는 book_id가 필요하다.
...
// 책의 정보를 넘겨주고 책의 정보를 수정하게 한다.
int result = bookDAO.updateBook(book, book_id, imageStatus);

PrintWriter pw = response.getWriter();

if(result == 1) { // 책의 정보가 수정이 완료되었으면
	pw.println("<script>");
	pw.println("alert('책의 정보를 수정하였습니다.')");
	pw.println("location.href='bookList.jsp?book_kind=" + book_kind + "'");
	pw.println("</script>");
} else { // 책의 정보를 수정하는 중에 문제가 생겼으면
	pw.println("<script>");
	pw.println("alert('책의 정보 수정을 완료하지 못하였습니다.\\n잠시 후에 다시 시도해 주십시오.')");
